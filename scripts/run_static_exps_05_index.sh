...
ROUND_DIR="${RESULTS_DIR}/static_exps_05"
CSV_DIR="${ROUND_DIR}/csv"
LOG_DIR="${ROUND_DIR}/logs"
FIG_DIR="${ROUND_DIR}/figures"
FIG_OUT_DIR="${ROOT_DIR}/figures/05_static_exps_analysis"

CONFIG_FOLDER="static_exps_05_index"
...
echo "[PLOT] Generating round-05 plots"
python3 "${ROOT_DIR}/scripts/static_exps_05_index_plots.py" \
  --csv "${CSV_DIR}/${RESULT_CSV_NAME}" \
  --outdir "${FIG_OUT_DIR}"

cp -f "${FIG_OUT_DIR}"/*.png "${FIG_DIR}/"
cp -f "${FIG_OUT_DIR}"/*.csv "${FIG_DIR}/" || true

echo "[DONE] Round-05 index grid experiments completed"
